...
use neurovlm::forward::{GenericModel, Specter2, load_constants, text_query};
...
impl AppConstants {
    fn new() -> Self {
        ...
        let (text_encoder, neuro_decoder, aligner, mask, l_reg_fus, r_reg_fus, title_embeddings) =
            out.unwrap();
        Self {
            text_encoder,
            neuro_decoder,
...
async fn handle_query(
    ...
    let result = task::spawn_blocking(move || -> Result<(Vec<i32>, Vec<f32>, Vec<f32>), String> {
        // Pass query and constants
        let (top_inds, surface_vec, img3d) = text_query(
            &payload.input,
        ...
        Ok((top_inds, surface_vec, volume_vec))
    })
    .await
    .map_err(|e| {
        (
